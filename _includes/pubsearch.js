document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleTag(evt) {
    const el =
      evt.target.tagName === "SPAN" ? evt.target.parentElement : evt.target;
    const parent = el.parentElement;
    const selected = parent.querySelectorAll(".tag.selected");

    toggle =
      el.classList.contains("toggleable") ||
      evt.ctrlKey ||
      evt.metaKey ||
  ...
  function search() {
    ...
    if (!results.length) {
      results = Object.entries(index.store).map(([id, doc]) => ({
        id,
        ...doc,
      }));
    } else {
    ...
    const selectedTypes = [...$$("#types .tag.selected")].map((e) =>
      e.getAttribute("data-tag").toLowerCase()
    );
    if (selectedTypes.length) {
      results = results.filter((r) => selectedTypes.indexOf(r.type) >= 0);
    }

    const selectedTags = [...$$("#tags .tag.selected")].map((e) =>
      e.getAttribute("data-tag").toLowerCase()
    );
    if (selectedTags.length) {
      results = results.filter((r) =>
        selectedTags.every((t) => r.tags.indexOf(t) >= 0)
      );
    }
  ...
  for (const elem of $$(".pub")) {
    try {
      id = elem.getAttribute("data-slug");
      tags = elem.getAttribute("data-tags").split(" | ");
      title = elem.getAttribute("data-title");
      year = elem.getAttribute("data-year");
      console.log(title, tags);
      index.add({ id: id, title: title, tags: tags, year: year });
    } catch (e) {
      console.error(e);
    }
  }
